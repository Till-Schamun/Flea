...
infile <- file.path(directory, "all_speed_dt035.csv")
df <- fread(infile)
print (df)
...
df <- df[, c("Frame", "speed", "heading", "VideoID", "FrameID")]
...
df %>% reframe(n = n(), .by = c(bat))
 
p_relativ <- ggplot(df, aes(x = round(as.numeric(percent),2), y = speed)) +
...
fit = lmer(speed~ as.numeric(total_weight)+(1|bat), data = df)
summary(fit)
plot(fit)
hist(df$speed)
