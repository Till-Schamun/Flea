...
for (i in seq_len(nrow(meta))) {
  Name_infile <- meta$`Intervall_file`[i]
  ...
  p1 <- ggplot(df_plot, aes(z, x, col = speed)) +
    ...
    scale_x_continuous(limits = c(-4.5, 4.5), name = "Z in [m]") +
    scale_y_continuous(limits = c(-3.5, 3.5), name = "X in [m]")
  ...
  p2 <- ggplot(df_plot, aes(z, y, col = speed)) +
    ...
    scale_color_viridis_c()+
    scale_x_continuous(limits = c(-4.5, 4.5), name = "Z in [m]") +
    scale_y_continuous(limits = c(-1.5, 1.5), name = "Y in [m]")
  ...
  p3 <- ggplot(df_plot, aes(x, y, col = speed)) +
    ...
    geom_text(data = df_plot[1, , drop = FALSE],
              ...
              vjust = -0.5, size = 3, inherit.aes = TRUE) +
    scale_color_viridis_c()+
    scale_x_continuous(limits = c(-3.5, 3.5), name = "X in [m]") +
    scale_y_continuous(limits = c(-1.5, 1.5), name = "Y in [m]")
  ...
  combined_plot <- ggpubr::annotate_figure(
    combined_plot,
    top = ggpubr::text_grob( paste0("example flight path of ", Bat_id )    )
  )
  print(combined_plot)
  ...
  #Save Plot
  ggsave(
    filename = paste0 (outfile,"_", format(Sys.time(),"%Y%m%d_%H%M%S"), "_interval_15", ".png"   ) ,   # Dateiname
